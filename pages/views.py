...
def search(request):
    ...
    if(select == 'title'):
        ...
        try:
            id = monograph[0].id_monography
            monograph = requests.get("http://localhost:8000/api/register-monograph/{}/?format=json".format(id))
            monograph = Monograph.objects.filter(title__icontains = monograph.json()['title'])
        except(IndexError):
            monograph=[]
        #monograph = Monograph.objects.filter(title__icontains = result)
    elif(select == 'course'):
        ...
        try:
            id = monograph[0].id_monography
            monograph = requests.get("http://localhost:8000/api/register-monograph/{}/?format=json".format(id))
            monograph = Monograph.objects.filter(course__icontains = monograph.json()['course'])
        except(IndexError):
            monograph=[]
        #monograph = Monograph.objects.filter(course__icontains = result)
        ...
        try:
            id = monograph[0].id_monography
            monograph = requests.get("http://localhost:8000/api/register-monograph/{}/?format=json".format(id))
            monograph = Monograph.objects.filter(university__icontains = monograph.json()['university'])
        except(IndexError):
            monograph=[]
        #monograph = Monograph.objects.filter(university__icontains = result)
        ...
        try:
            id = monograph[0].id_monography
            monograph = requests.get("http://localhost:8000/api/register-monograph/{}/?format=json".format(id))
            monograph = Monograph.objects.filter(key_words__icontains = monograph.json()['key_words'])
        except(IndexError):
            monograph=[]
        #monograph = Monograph.objects.filter(key_words__icontains = result)
        ...
        try:
            id = monograph[0].id_monography
            monograph = requests.get("http://localhost:8000/api/register-monograph/{}/?format=json".format(id))
            monograph = Monograph.objects.filter(abstract__icontains = monograph.json()['abstract'])
        except(IndexError):
            monograph=[]
        #monograph = Monograph.objects.filter(abstract__icontains = result)
    elif(select == 'author'):
